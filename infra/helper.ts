class Helper{ //Respeitando o princípio do solid com responsabilidade única
    //Método genérico 
    sendResponse = function (res, statusCode, data){
        res.status(statusCode).json({resultado: data, status: true});
    }

    sendExtrato = function (res, statusCode, data){
        res.status(statusCode).json({Extrato: data});
    }

    sendNaoEncontrado = function (res, statusCode, data){
        res.status(statusCode).json({status: false, resultado:'não encontrado'});
    }

    sendCartao = function (res, statusCode, data){
        res.status(statusCode).json({resultado: data, status: true});
    }

    sendNaoEncontradoMsgPersonalizada = function (res, statusCode, data){
        res.status(statusCode).json({resultado: data, status: false});
    }

    sendErro = function (res, statusCode, data){
        res.status(statusCode).json({descricao: data, status: false});
    }

    sendLista = function (res, statusCode, data){
        res.status(statusCode).json({lista: data});
    }

    sendFalhaCadastro = function (res, statusCode, data){
        res.status(statusCode).json({status: false});
    }

    sendFalha = function (res, statusCode, data){
        res.status(statusCode).json({status: false});
    }

    sendSaldo = function (res, statusCode, data){
        res.status(statusCode).json({saldo: data, status: true});
    }

    sendTrataCampo = function (res, statusCode, data){
        res.status(statusCode).json({mensagem: data, retorno: false});
    }    

    getToken = function (res, statusCode, data){
    ...
    gerarCodVerificacao = function (min, max){ 
        min = Math.ceil(min);
        max = Math.floor(max);
    ...
    tratarCamposObrigatorios = (obj, arrExcessao) =>{
        let retorno = '';
        for(var i in obj){
