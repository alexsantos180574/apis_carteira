...
function executarEndPointIugu(res, pUrl, pData, pMethod) {
...
function gerarBoletoIugu(obj, res, client_id) {
    return __awaiter(this, void 0, void 0, function* () {
        const tokenProducao = '8d77fcdc4047d7fd1f208d8b03f735a7';
        const tokenTeste = '92a1f275c5abcd2834831a0b995b4fa7';
        const datavenc = yield funcoes_1.retornaDataVencimentoBoleto()
            .then(ret => {
            return ret;
        });
        mdlTransacoes_1.obterDadosCliente(res, obj)
            .then(retorno => {
            //console.log(retorno);
            var axios = require('axios');
            ...
            executarEndPointIugu(res, url, data, Method)
                ...
                mdlTransacoes_1.gravaBoletoGerado(res, dados, client_id);
            })
                .catch(function (error) {
...
class ctrTransacoes {
    ...
    listarBoletosGeradosIugu(req, res) {
        ...
        funcoes_1.validarInformacoes(res, req, arrExcessao)
            .then(msg => {
            if (msg.retorno == 'OK') {
                mdlTransacoes_1.listaBoletosGerados(res, obj, msg.client_id);
            }
            else {
                res.status(400).send({ erro: msg.retorno, status: false });
            }
