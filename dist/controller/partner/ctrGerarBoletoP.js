...
function emitirBoletoP(res, obj, token) {
    return __awaiter(this, void 0, void 0, function* () {
        var nome;
        var celular;
        var email;
            ...
            WHERE c.ID_CLIENTE_TITULAR = e.ID_CLIENTE
                ...
                sql.query(strSql, function (err, results) {
                    ...
                    axios(config)
                        ...
                        const objeto = JSON.parse(JSON.stringify(response.data));
                        const resultado = objeto;
                        inserirDadosBoletoP(res, obj, results.recordset[0], resultado);
                    })
                        .catch(function (error) {
                        console.log(error.response.data.message);
                        helper_1.default.sendErro(res, 401, 'Falha ao emitir o Boleto. Erro: ' + error.response.data.message);
                    });
                });
            }
        });
...
function inserirDadosBoletoP(res, obj, rs, retorno) {
    //console.log(retorno);
    var valor = obj.valor;
    ...
    sql.connect(helper_1.default.configSql, function (err) {
        ...
        request.query(strSql, function (err, data) {
            if (err) {
                helper_1.default.sendErro(res, 401, 'Falha ao emitir o Boleto.');
                console.log(err);
            }
            else {
                helper_1.default.sendResponse(res, 201, { resultado: 'Boleto gerado com sucesso!', qrCode: qrcode, codigobarra: codigo_de_barras, codigo: token_id_boleto, datavencimento: datavenc });
            }
        });
    });
}
class CtrGerarBoletoP {
    createP(req, res) {
        let obj = req.body;
        var validarDados = '';
        ...
        if (validarDados) {
            gerarTokenP(res, obj)
                ...
                emitirBoletoP(res, obj, token)
                    ...
                    .catch(Error => helper_1.default.sendErro(res, 401, 'Falha ao emitir o Boleto.'));
            })
                .catch(Error => helper_1.default.sendErro(res, 401, 'Falha ao gerar o token.'));
        }
        else {
            helper_1.default.sendErro(res, 401, 'O campo ' + validarDados + ' é necessário!');
        }
    }
}
exports.default = new CtrGerarBoletoP();
